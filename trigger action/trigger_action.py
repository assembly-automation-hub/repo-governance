...
def was_already_closed(title_keyword: str) -> bool:
    ...
    Args:
        title_keyword (str): A case-insensitive substring used to compare against
            titles of closed repository issues.

    Returns:
        bool: ``True`` if a closed issue contains ``title_keyword`` in its
...
def build_permalink(filename: str, line: int = 1) -> str:
    ...
    Args:
        filename (str): The repository-relative path to the file being linked.
        line (int, optional): The 1-based line number to anchor in the GitHub
            URL. Defaults to ``1``.

    Returns:
        str: A GitHub URL pointing to the requested file and line for the
            active commit or pull request head SHA.
...
def call_model(prompt: str, retries: int = 3, delay: int = 5) -> dict:
    ...
    Args:
        prompt (str): The fully rendered instruction payload describing the
            repository context and diff to analyze.
        retries (int, optional): The maximum number of request attempts to make
            before exiting gracefully. Defaults to ``3``.
        delay (int, optional): The number of seconds to wait between failed
            attempts. Defaults to ``5``.

    Returns:
        dict: The parsed JSON object returned by the model, containing
            issue metadata such as title, severity, body, and summary fields.

    Raises:
        requests.HTTPError: If ``requests.post`` receives a non-success response
            during an individual attempt. The exception is caught internally,
            logged, and retried until the retry budget is exhausted.
        json.JSONDecodeError: If the model returns invalid JSON. The exception is
            caught internally, logged, and retried until the retry budget is
            exhausted.
